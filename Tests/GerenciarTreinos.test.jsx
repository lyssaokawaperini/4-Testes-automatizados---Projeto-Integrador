...
describe("Componente GerenciarTreinos", () => {
  ...
  test("deve exibir 'Carregando treinos...' inicialmente e depois a lista de treinos", async () => {
    const mockTrainingData = [
      {
        ...
        modalityName: "CS2",
        modalityTag: "CS",
        description: "Treino tático mapa Dust II",
        startTimestamp: "2025-06-10T10:00:00.000Z",
        endTimestamp: "2025-06-10T12:00:00.000Z",
        status: "SCHEDULED",
        attendedPlayers: [{ _id: "u1", name: "Jogador Alfa" }],
      },
    ];
    ...
    fetchExternalModalities.mockResolvedValueOnce({
      mod1: { _id: "m1", Name: "Counter-Strike 2", Tag: "CS2" },
    });
    fetchUsers.mockResolvedValueOnce([
      { _id: "u1", name: "Jogador Alfa", email: "alfa@teste.com" },
    ]);
    renderComponent();
  ...
  test("deve exibir 'Nenhum treino encontrado.' se não houver treinos após o carregamento", async () => {
    ...
    renderComponent();
    expect(
      await screen.findByText("Nenhum treino encontrado.")
    ).toBeInTheDocument();
  });
  ...
  test("deve exibir mensagem de erro se o carregamento de treinos falhar", async () => {
    ...
    renderComponent();
    expect(
      await screen.findByText(/Falha ao carregar treinos: Falha épica na API/i)
    ).toBeInTheDocument();
  });
  ...
  test("deve abrir o modal 'Adicionar Novo Treino', permitir preenchimento e fechar ao cancelar", async () => {
    ...
    const participantSelect = screen.getByDisplayValue(
    ...
    expect(
      screen.queryByRole("heading", { name: "Adicionar Novo Treino" })
  ...
  test("deve salvar um novo treino com sucesso ao submeter o formulário", async () => {
    ...
    await user.type(
      ...
      newTrainingDataFromApi.description
    );
    ...
    await user.selectOptions(
      screen.getByDisplayValue("Adicionar participante..."),
      "playerZ"
    );

    await user.click(screen.getByRole("button", { name: "Criar Treino" }));

    const start = new Date("2025-07-01T14:00").toISOString();
    const end = new Date("2025-07-01T16:00").toISOString();

    expect(createTraining).toHaveBeenCalledWith(
    ...
    expect(
      await screen.findByText(newTrainingDataFromApi.description)
    ).toBeInTheDocument();
    expect(
      screen.getByText(newTrainingDataFromApi.modalityName)
    ).toBeInTheDocument(); // ou Tag
  });
  ...
  test("deve exibir mensagem de erro no modal se a criação do treino falhar", async () => {
    ...
    fetchTrainings.mockResolvedValue([]);
    fetchExternalModalities.mockResolvedValueOnce({
      idModY: { _id: "idModY", Name: "Basquete", Tag: "NBA" },
    });
    fetchUsers.mockResolvedValueOnce([]);
    ...
    fireEvent.change(screen.getByLabelText(/Data\/Hora de Início:/i), {
      target: { value: "2025-08-01T10:00" },
    });
    fireEvent.change(screen.getByLabelText(/Data\/Hora de Término:/i), {
      target: { value: "2025-08-01T11:00" },
    });

    await user.click(screen.getByRole("button", { name: "Criar Treino" }));
    expect(
      await screen.findByText(/Erro ao criar treino: Erro ao salvar no banco/i)
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: "Adicionar Novo Treino" })
    ).toBeInTheDocument();
  });
});
